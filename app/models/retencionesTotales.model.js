...
Retencion.create = (newRetencion, result) => {
  let sql =
    "INSERT INTO retenciones(periodo, agente, contribuyente, numeroInscripcion, base, certificado, retenido) VALUES(?,?)";
  // first row only
  database.appDatabase.all(
    ...
    [
      ...
      newRetencion.agente,
      newRetencion.contribuyente,
      newRetencion.numeroInscripcion,
      newRetencion.base,
      newRetencion.certificado,
      newRetencion.retenido,
    ],
    (err, res) => {
      if (err) {
        result(null, err);
...
Tutorial.remove = (periodo, result) => {
  database.appDatabase.all(
    "DELETE FROM tutorials WHERE periodo = ?",
    periodo,
    (err, res) => {
      if (err) {
